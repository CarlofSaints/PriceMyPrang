...
export async function POST(request: Request) {
  ...
  // A temporary password is generated by us from an alphabet with no spaces in
  // it, so whitespace around one can only have come from the copy-paste that
  // delivered it. Accept the trimmed form rather than reject a password the
  // person did receive correctly. Never done for a self-chosen password — that
  // one may legitimately contain spaces.
  const supplied =
    user.mustChangePassword && !(await verifyPassword(currentPassword, user.passwordHash))
